#!/usr/bin/env python3
"""
Script to update AdminPanel products display:
1. Remove inline description
2. Add description link that opens modal
3. Keep vendor names
"""
...
# Find and replace the product info section to add description modal
...
# Add description column after product name - need to find the table header and add column
# First, let's add the description link in the product row
old_categoria_cell = """                                <td className="admin-table-cell">{producto.categoria?.nombre || 'Sin categoría'}</td>"""
...
content = content.replace(old_categoria_cell, new_with_description)

# Write the file back
with open('frontend/src/pages/admin/AdminPanel.jsx', 'w', encoding='utf-8') as f:
    f.write(content)

print("✅ Successfully updated product description display")
print("   - Removed inline description")
print("   - Added 'Ver descripción' link with modal")
